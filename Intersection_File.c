...
FONTCHARACTER * CharToFonts1(const char *cFileName,FONTCHARACTER *fFileName);
char * FontToChars1(const FONTCHARACTER *fFileName,char *cFileName);
...
//						File Directory
///////////////////////////////////////////////////////////////
int CreateDirectorys1()
{
	FONTCHARACTER f_path_name[20];
	Bfile_CreateDirectory(CharToFonts1("\\\\crd0\\JDF",f_path_name));
}

char szSrdDir1[]="\\\\crd0\\JDF";
//char szSrdDir1[]="\\\\fls0\\JDF";

///////////////////////////////////////////////////////////////
//							File List
///////////////////////////////////////////////////////////////
int SelectJDFFiles (char * file_name)
// -1 no file , 1 complate ,0 cancel
{
	...
	char *files;
	FILE_INFO file_info;
//	CharToFont("\\\\fls0\\JDF\\*.jdf",find_path);
	sprintf(str,"%s\\*.jdf",szSrdDir1);
	CharToFonts1(str,find_path);
	if(Bfile_FindFirst (find_path,&find_h,find_name,&file_info)==0)  //函数搜索目录中文件的名称符合指定的文件名
	{
		...
		for(;;)
		{
			...
			if(ret!=0)  //if 
				break;
			strncpy((files+13*i),FontToChars1(find_name,str),13);
			if(strcmp((files+13*i),setup_data.jdfroute_name)==0) //文件选中后，现在记忆位置
			index=i; 
			i++;
...
int TestJDFFile (const char *file_name_c_a)
{
	FONTCHARACTER file_name_f[30];
	char file_name_c[30];
	int file_h;

	sprintf (file_name_c,"\\\\crd0\\JDF\\%s",file_name_c_a);	

	CharToFonts1(file_name_c,file_name_f);
...
int DelJDFFile (const char *file_name_c_a)
{
	FONTCHARACTER file_name_f[30];
	char file_name_c[30];
	int file_h;
...
int SaveJDFFile (const char *file_name_c_a)
{
	FONTCHARACTER file_name_f[30];
	char file_name_c[30];
	int file_h;
	...
	jdfroute.size * sizeof(JDFPOINT) + sizeof (JDFROUTE));
	file_h = Bfile_OpenFile (file_name_f,_OPENMODE_WRITE);
	
	Bfile_SeekFile (file_h,0);
	Bfile_WriteFile (file_h,&jdfroute,sizeof(JDFROUTE));
	
	Bfile_SeekFile (file_h,sizeof(JDFROUTE));
	Bfile_WriteFile (file_h,jdfroute.set,jdfroute.size * sizeof(JDFPOINT));

	Bfile_CloseFile (file_h);
	return 0;	
}

int LoadJDFFile (const char *file_name_c_a)    
{
	FONTCHARACTER file_name_f[30];
	char file_name_c[30];
	int file_h;
	...
	if(file_h>=0){
	    ...
	    Bfile_ReadFile(file_h,&jdfroute,sizeof(JDFROUTE),0);
	    jdfroute.set = (JDFPOINT*)calloc(jdfroute.size,sizeof(JDFPOINT));
	    Bfile_ReadFile(file_h,jdfroute.set,sizeof(JDFPOINT)*jdfroute.size,sizeof(JDFROUTE));
...
static FONTCHARACTER IniSRDFile[] = { '\\','\\','c','r','d','0','\\','j','d','f','.','i','n','i' ,'\0'} ;
...
void SaveIniJDFFile()
{
	...
	if (ptr >= 0) {
		Bfile_WriteFile(ptr, setup_data.jdfroute_name, ln) ;
		Bfile_CloseFile(ptr) ;
	}
}

int LoadLastDataJDFFile()
{
	char strFileName[50]; 
	int reg_ptr = -1;
	int ln;
	if(setup_data.jdfroute_name[0]==0)
	{
		...
		if (reg_ptr >= 0) {
			...
			if(ln>0)
			{   
				if(ln>=sizeof(setup_data.jdfroute_name)-1)
					ln=sizeof(setup_data.jdfroute_name)-1;
				Bfile_ReadFile(reg_ptr, setup_data.jdfroute_name,ln,0) ;
				setup_data.jdfroute_name[ln]=0;
			}
			...
			LoadJDFFile(setup_data.jdfroute_name);
			return 1;
		}		
	}
	return 0;
