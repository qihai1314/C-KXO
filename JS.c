...
int ZS()
{
	...
	pi=Pa+(Pb-Pa)*fabs(yscs.k-DKa)/fabs(DKb-DKa);  //计算里程处曲率
	J_out=Ca+0.5*(Pa+pi)*fabs(yscs.k-DKa);  //计算里程处切线方位角
	h=fabs(yscs.k-DKa)/n;
    W==0;
	z==0;
	U==0;
...
{
	dk=DKa+k*h/2;
	p=Pa+(Pb-Pa)*fabs(dk-DKa)/fabs(DKb-DKa);
	c=Ca+0.5*(Pa+p)*fabs(dk-DKa);
	if (k%2==0){
	W=W+cos(c);
	z=z+sin(c);
	}
	...
	yscs.ye = Ya+h/6.0*(sin(Ca)+4.0*v+2.0*z+sin(J_out)); 
	yscs.fn=J_out/PI*180;  //将弧度转换角度      弧度=角度乘以π后再除以180   角度=弧度除以π再乘以180
	zfset.ks=yscs.k;
	return 1;	
}
...
int FS()
{
	...
	do{
		...
		fj = fj/ PI * 180.0;  //将弧度转换角度
		if (yscs.xn -yscs.x1< 0.0)	fj += 180.0;  //fj=fj+180
		if (fj<0.0)	fj += 360.0;                 //fj=fj+360
		fw=fi*sin((yscs.fn-fj-90.0)*PI/180.0);  //将角度转换弧度
		yscs.k=yscs.k+fw;
		++cc;
		if (cc>=80)	fw=0.00001;
...
int GC()
{
	...
	int i=0;
	if (yscs.k<l_sqx[0].s_qdk)	//SQX_ERR();
	while(yscs.k<l_sqx[i].s_qdk || yscs.k>l_sqx[i].s_zdk){
		i++;
	}
	
	ge=l_sqx[i].s_qdi/100;   //前坡
	gf=l_sqx[i].s_zdi/100;   //后坡
	ga=l_sqx[i].s_bpk;   //变坡点桩号
	gz=l_sqx[i].s_bph;   //变坡点高程
	gr=l_sqx[i].s_r;     //变坡点半径
	gt=gr*fabs(gf-ge)/2; //切线长
	if (gf-ge>0)	gw=1;	else gw=-1; 
	if (yscs.k<ga)	gi=-ge;	else gi=gf;
	gp=fabs(ga-yscs.k); //计算点距离变坡点长度
